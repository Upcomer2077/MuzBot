...
async def help(message: Message, command: CommandObject):
    ...
    if ARGS.strip().find(COMSET.FORCE.value) != -1:
        return message.answer(
            ...
            f"Пример: /{COMSET.FORCE.value} https://music.youtube.com/watch?v=qdCU4ReBROc \n"
    ...
    if ARGS.strip().find(COMSET.PLAYLIST.value) != -1:
        return message.answer(
            f"*{COMSET.PLAYLIST.value}*: команда для скачивания плейлиста по ссылке. Аналогична команде /{COMSET.FORCE.value}, но для плейлистов.\n"
    ...
    if ARGS.strip().find(COMSET.UNSUBSCRIBE.value) != -1:
        return message.answer(
            f"*{COMSET.UNSUBSCRIBE.value}*: удаляет подписку на исполнителя.\n"
    ...
    if ARGS.strip().find(COMSET.SUBSCRIBE.value) != -1:
        return message.answer(
            f"*{COMSET.SUBSCRIBE.value}*: позволяет подписаться на исполнителя. Каждые несколько дней бот проверяет релизы исполнителей и при появлении новых отправляет их в чат\n"
