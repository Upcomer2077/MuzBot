...
@router.callback_query(PaginationSubsArtistsCallback.filter())
async def process_pagination(
    callback: CallbackQuery,
    callback_data: PaginationSubsArtistsCallback,
    state: FSMContext,
):
    ...
    content, _nav_markup, _drop_builder, start_idx = U.get_page_content(
        subs, page=target_page, pag_cb_type=PaginationSubsArtistsCallback, max=0
    )
