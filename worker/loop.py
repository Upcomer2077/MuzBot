...
async def playlist_worker_loop():
    ...
    while True:
        async with PLAYLIST_QUEUE as task:
            LOGGER.info(f"🚀Worker took task on playlist {task.playlist_id}")

            tracks_info = await DM.summon_slaves_from_playlist(task.playlist_id)
            if not tracks_info:
                LOGGER.error(
                    f"Can not find info about playlist {task.playlist_id} in worker loop."
                )
                return await bot.bot.send_message(
                    task.user_id, "Не удалось скачать плейлист. Повторите попытку."
                )
            ...
            async for cache_data_list in chunked_downloader(non_cached_tracks):
                ...
                for t in audio_non_cached:
                    ...
                    TS = TrackState().fill_from(a)
                    try:
                        TS.sent_audio = await bot.bot.send_audio(
                            CHANNEL_STORAGE_ID,
                            a_file,
                            thumbnail=thumbnail,
                            title=a.title,
                        ...
                        if e.message.find("Request Entity Too Large") != -1:
                            ...
                            await bot.bot.send_message(
                                task.user_id,
                                f"Невозможно отправить трек {TS.title}. Файл слишком большой",
                            )
                        LOGGER.error(f"Network error: {e}")
                    finally:
                        await finalize_download(a.video_id, TS)
        ...
        LOGGER.info(f"🏁 End of task on playlist {task.user_id}")
        await asyncio.sleep(1)
